...
let OUTDIR = pth.join(common.OUTDIR, "fs");
...
describe("TemDir", () => {
    it("Initialize temdirs", () => {
        common.prepare("fs");
        for (const dir of [TEMDIR, TEMDIR1, TEMDIR2, NOT_A_TEMDIR, NOT_A_TEMDIR2])
            fs.mkdirSync(dir);
        temdir = TemDir.init(TEMDIR);
        assert.ok(fs.lstatSync(temdir.path).isDirectory());
        temdir1 = TemDir.init(TEMDIR1);
        temdir2 = TemDir.init(TEMDIR2);
    });
    it("constructor", () => {
        // Valid temdir
        new TemDir(TEMDIR);
